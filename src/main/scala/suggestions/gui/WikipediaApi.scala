...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs.map(string => string.replaceAll(" ", "_"))
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      Observable.amb(
        obs.onErrorReturn[Try[T]](Failure(_)),
        obs.map(Success(_))
      )
    }
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      val timeout = Observable.timer(totalSec seconds).map(timeElapsed => timeElapsed == 0L)
      for {
        timeoutReached <- timeout
        value <- obs.takeWhile(_ => timeoutReached)
      } yield value
    }
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      for {
        value <- obs
        poweredValue <- requestMethod(value).recovered
      } yield poweredValue
    }
