...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = {
      Observable.create[String] {
        observer => {
          field.subscribe { case ValueChanged(event) => observer.onNext(event.field.text) }
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = {
      Observable.create[Button] {
        observer => {
          button.subscribe { case ButtonClicked(event) => observer.onNext(event.button) }
          Subscription(observer)
        }
      }
    }
